...
import { Package, DollarSign, AlertTriangle, Download, Calendar, BarChart3, ArrowLeft, Users, Clock, RefreshCw } from "lucide-react"
import DashboardHeader from "@/components/dashboard-header"
import { reportesApi, type ReporteVentas, type ReporteInventario, type ReporteHorarios } from "@/lib/api"

// IMPORTAMOS TUS FUNCIONES DE EXPORTACIÓN (Del archivo que ya creaste)
import { 
  exportVentasDiarias, 
...
export default function ReportesPage() {
  ...
  // --- LÓGICA DE EXPORTACIÓN CONECTADA ---
  const exportarReporte = (tipo: string) => {
    ...
    if (tipo === "ventas-dia") {
        ...
        if (reporteInventario?.inventario_por_categoria) {
            exportInventarioCategoria(reporteInventario.inventario_por_categoria)
        }
    }
    else if (tipo === "asistencia") {
        if (reporteHorarios?.asistencia_por_usuario) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        {/* FILTROS DE PERÍODO (TU DISEÑO ORIGINAL) */}
        <Card>
          <CardHeader>
        ...
        {isLoading ? (
          ...
          <>
            {/* KPIs PRINCIPALES (TU DISEÑO ORIGINAL) */}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-600">
                        {formatCLP(reporteInventario?.valor_inventario_total || 0)}
                      </p>
                      <p className="text-sm text-gray-500 mt-1">
            ...
            <Tabs defaultValue="ventas" className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => exportarReporte("ventas-dia")}>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {reporteVentas?.ventas_por_vendedor && reporteVentas.ventas_por_vendedor.length > 0 ? (
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b">
                            ...
                            <th className="text-right p-3 font-medium text-gray-700">Total Vendido</th>
                          </tr>
                        </thead>
                        <tbody>
                          {reporteVentas?.productos_mas_vendidos && reporteVentas.productos_mas_vendidos.length > 0 ? (
                            reporteVentas.productos_mas_vendidos.map((producto, index) => (
                              <tr key={producto.producto_nombre} className="border-b hover:bg-gray-50">
              ...
              <TabsContent value="inventario" className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => exportarReporte("inventario-categoria")}>
                          <Download className="h-4 w-4 mr-2" />
                          Exportar
                        </Button>
                      </div>
                    ...
                    <CardContent>
                      {reporteInventario?.inventario_por_categoria && reporteInventario.inventario_por_categoria.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {reporteInventario?.productos_mayor_valor && reporteInventario.productos_mayor_valor.length > 0 ? (
                          reporteInventario.productos_mayor_valor.slice(0, 5).map((producto, index) => (
                            <div
                              key={producto.id}
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                ...
                                <p className="text-sm text-gray-600 mt-1">{producto.stock} unidades</p>
                              </div>
                            </div>
                          </div>
                        ))
              ...
              <TabsContent value="horarios" className="space-y-6">
                ...
                {/* Asistencia por Usuario (CON BOTÓN DE EXPORTAR AÑADIDO) */}
                <Card>
                  <CardHeader className="flex flex-row justify-between items-center">
                    <CardTitle>Asistencia por Usuario</CardTitle>
                    <Button variant="outline" size="sm" onClick={() => exportarReporte("asistencia")}>
                        <Download className="h-4 w-4 mr-2" />
                        Exportar
                    </Button>
                  </CardHeader>
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b">
                            ...
                            <th className="text-right p-3 font-medium text-gray-700">Días Completos</th>
                            <th className="text-right p-3 font-medium text-gray-700">Promedio Llegada</th>
                            <th className="text-center p-3 font-medium text-gray-700">Cumplimiento</th>
                          </tr>
                        </thead>
                        <tbody>
                          {reporteHorarios?.asistencia_por_usuario && reporteHorarios.asistencia_por_usuario.length > 0 ? (
                            reporteHorarios.asistencia_por_usuario.map((usuario) => {
                              const porcentaje = usuario.total_dias > 0 
                                ? Math.round((usuario.dias_completos / usuario.total_dias) * 100)
                                : 0

                              return (
                                <tr key={usuario.usuario_id} className="border-b hover:bg-gray-50">
                                  <td className="p-3 font-medium">{usuario.usuario_nombre}</td>
                                  <td className="p-3 text-right">{usuario.total_dias}</td>
                                  <td className="p-3 text-right">{usuario.dias_completos}</td>
                                  <td className="p-3 text-right">{usuario.promedio_llegada}</td>
